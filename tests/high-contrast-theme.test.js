...
test("모바일 종목 상세창은 선명한 CEO 카드와 한 화면용 컴팩트 거래 흐름을 사용한다", async () => {
  const css = await read("high-contrast-theme.css");
  assert.match(css, /@media\s*\(max-width:\s*767px\)[\s\S]*?\.stock-detail-modal-card\s*\{[^}]*max-height:\s*100dvh[^}]*padding:\s*10px/i);
  assert.match(css, /\.stock-detail-character\s*\{(?=[^}]*grid-template-columns:\s*80px\s+minmax\(0,\s*1fr\))(?=[^}]*min-height:\s*96px)[^}]*\}/i);
  assert.match(css, /\.stock-detail-character\s*>\s*\.sector-ceo\s*\{[^}]*position:\s*relative[^}]*width:\s*80px[^}]*height:\s*96px[^}]*background-size:\s*300%\s+100%/i);
  assert.match(css, /\.stock-detail-character-copy\s+h2\s*\{[^}]*font-size:\s*1\.2rem[^}]*text-overflow:\s*ellipsis/i);
  assert.match(css, /\.stock-detail-sector-switcher\s*\{[^}]*margin-top:\s*6px[^}]*padding:\s*6px/i);
  assert.match(css, /\.stock-detail-sector-list\s+button\s*\{[^}]*min-height:\s*34px[^}]*max-width:\s*132px/i);
  assert.match(css, /\.stock-detail-body\s+\.chart-wrap\s*\{[^}]*height:\s*120px[^}]*min-height:\s*120px/i);
  assert.match(css, /\.stock-detail-body\s+\.selected-position-grid\s*>\s*div\s*\{[^}]*min-height:\s*44px/i);
