...
test("두 사용자가 서버 권한 게임과 비공개 포트폴리오를 공유한다", { timeout: 12_000 }, async (context) => {
  ...
  const assetAction = await json(`${base}/api/rooms/${survivalCode}/action?token=${quickMode.token}`, { method: "POST", body: JSON.stringify({ type: "mvp-action", payload: { type: "buy", assetKey: "gold", quantity: 1 } }) });
