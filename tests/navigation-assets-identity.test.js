...
test("대체자산 카드는 이름과 수량 및 증감 버튼이 서로 겹치지 않는다", async () => {
  const css = await read("high-contrast-theme.css");
  assert.match(css, /\.alternative-assets\s*>\s*div\s*\{(?=[^}]*grid-template-columns:\s*24px\s+minmax\(0,\s*1fr\))(?=[^}]*grid-template-rows:\s*auto\s+auto\s+auto)[^}]*\}/i);
  assert.match(css, /\.alternative-assets\s*>\s*div\s*>\s*\.asset-icon\s*\{[^}]*grid-column:\s*1[^}]*grid-row:\s*1/i);
  assert.match(css, /\.alternative-assets\s*>\s*div\s*>\s*b\s*\{[^}]*grid-column:\s*2[^}]*text-overflow:\s*ellipsis/i);
  assert.match(css, /\.alternative-assets\s*>\s*div\s*>\s*small\s*\{[^}]*grid-column:\s*1\s*\/\s*-1/i);
  assert.match(css, /\.alternative-assets\s*>\s*div\s*>\s*i\s*\{[^}]*grid-column:\s*1\s*\/\s*-1[^}]*width:\s*100%/i);
  assert.match(css, /\.alternative-assets\s*>\s*div\s*>\s*i\s+button\s*\{[^}]*width:\s*100%[^}]*min-width:\s*0/i);
});
