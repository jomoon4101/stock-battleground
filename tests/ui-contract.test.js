...
test("생존 상태·선택 종목·보유 종목·정보 카드와 명확한 거래 계약이 존재한다", async () => {
  ...
  assert.match(engine, /playerMax: 6/);
  ...
  assert.match(engine, /long: Object\.freeze\(\{ totalTurns: 30[\s\S]*playerCount: 6[\s\S]*stockCount: 11/);
...
test("모바일 우선 섹터 카드·CEO 3상태·자동 모드 계약이 존재한다", async () => {
  ...
  assert.match(html, /10라운드 · 11섹터/);
  assert.match(html, /20라운드 · 11섹터/);
  assert.match(html, /30라운드 · 11섹터/);
