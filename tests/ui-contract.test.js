...
test("생존 상태·선택 종목·보유 종목·정보 카드와 명확한 거래 계약이 존재한다", async () => {
  const [html, app, styles, engine, server] = await Promise.all([
    readFile(`${root}/index.html`, "utf8"), readFile(`${root}/app.js`, "utf8"), readFile(`${root}/styles.css`, "utf8"), readFile(`${root}/engine.js`, "utf8"), readFile(`${root}/server.mjs`, "utf8"),
  ]);
...
test("모바일 우선 섹터 카드·CEO 3상태·자동 모드 계약이 존재한다", async () => {
  ...
  const [html, app, engine, mobileCss, ...ceoAssets] = await Promise.all([
    readFile(`${root}/index.html`, "utf8"), readFile(`${root}/app.js`, "utf8"), readFile(`${root}/engine.js`, "utf8"),
...
test("게임은 이전 버전 제한시간과 자동 턴 진행을 사용한다", async () => {
  const [html, app, engine, server, mobileCss] = await Promise.all([
    readFile(`${root}/index.html`, "utf8"), readFile(`${root}/app.js`, "utf8"), readFile(`${root}/engine.js`, "utf8"), readFile(`${root}/server.mjs`, "utf8"), readFile(`${root}/mobile-first.css`, "utf8"),
  ]);
...
test("대형 드래그 섹터 카드·등급 색상·뉴스 티커·종목 상세 거래창이 존재한다", async () => {
  const [html, app, mobileCss] = await Promise.all([
    readFile(`${root}/index.html`, "utf8"), readFile(`${root}/app.js`, "utf8"), readFile(`${root}/mobile-first.css`, "utf8"),
  ]);
...
test("거래 섹터 전환·명시적 카드 버튼·찌라시 알림·공시 링크·내 목표가 존재한다", async () => {
  ...
  assert.match(html, /app\.js\?v=20260702-25/);
...
test("모든 참가자가 공유하는 하단 전체 채팅 계약이 존재한다", async () => {
  const [html, app, server, mobileCss, aiChat] = await Promise.all([
    readFile(`${root}/index.html`, "utf8"), readFile(`${root}/app.js`, "utf8"), readFile(`${root}/server.mjs`, "utf8"), readFile(`${root}/mobile-first.css`, "utf8"), readFile(`${root}/ai-chat.js`, "utf8"),
  ]);
...
test("진행 중 서바이벌 바로 참여 목록과 한 줄 차트 타이틀이 존재한다", async () => {
  const [html, app, styles, server] = await Promise.all([
    readFile(`${root}/index.html`, "utf8"), readFile(`${root}/app.js`, "utf8"), readFile(`${root}/styles.css`, "utf8"), readFile(`${root}/server.mjs`, "utf8"),
  ]);
...
test("v2 밝은 입력 표면과 최소 가독성 크기 계약이 존재한다", async () => {
  const [styles, app] = await Promise.all([
    readFile(`${root}/styles.css`, "utf8"), readFile(`${root}/app.js`, "utf8"),
  ]);
  assert.match(styles, /2026-07 v2 readability and light-surface hardening/);
  assert.match(styles, /--font-body: 16px/);
  assert.match(styles, /--font-note: 14px/);
  assert.match(styles, /input, select, textarea, option[\s\S]*background: var\(--surface\)/);
  assert.match(styles, /\.rules-grid b \{ font-size: 18px/);
  assert.match(styles, /\.rules-grid p, \.data-note, \.item-modal-card > p \{ font-size: 16px/);
  assert.match(styles, /\.order-form select, \.order-form input, \.inline-form input[\s\S]*background: var\(--surface\)/);
  assert.match(styles, /@media \(max-width: 520px\)[\s\S]*\.modal h2 \{ font-size: 24px/);
  assert.match(app, /Math\.max\(0\.9, Math\.min\(1\.25/);
});
